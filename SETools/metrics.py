...
def compute_SDR(clean_signal,clean_predicted, noisy_signal,noisy_predicted, framerate = 8000):
    ...
    references = np.array([clean_signal[:min_length_a],noisy_signal[:min_length_b]])

    estimations = np.array([clean_predicted[:min_length_a],noisy_predicted[:min_length_b]])
...
def compute_SDR_single(references, estimates, framerate=8000):
    ...
    print("dddd")
    sdr, _, _, _ = mir_eval.separation.bss_eval_sources(references[:min_length], estimates[:min_length])
    return sdr[0]
